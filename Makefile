...
all:
 ...
 test - run all tests\n\
 pack - make output archive, file name format \"fields_vX.Y.Z_BRANCHNAME.tar.gz\"\n\
"
...
pack: make_build
	rm -f fields-*.tar.gz
	echo Create archive \"fields-$(VERSION)-$(BRANCH).tar.gz\"
	cd make_build; tar czf ../fields-$(VERSION)-$(BRANCH).tar.gz fields
...
fields.tar.gz: build
	cd make_build; tar czf ../fields.tar.gz fields && rm -rf ../make_build
